...
########################
# Collecting the SD160 #
########################

feat_names = ['valence','year','acousticness','danceability','duration_ms',
              'energy','explicit','id','instrumentalness','key','liveness','loudness',
              'mode','popularity','release_date','speechiness','tempo']
song_names = ['name','artists']
tracks_names = pd.read_csv('data.csv').iloc[:170653,[14,3]]
tracks_features = pd.read_csv('data.csv').iloc[:170653,[0,2,4,5,6,9,10,11,12,13,17,18]]

# Converting to floats and keeping only the relevant features
col_title = ["acousticness", "danceability", "duration_ms", "energy",
...
# Assembling the data
list_artists = []
for i in range(len(tracks_names["artists"])):
    artists = tracks_names["artists"][i].split("[")[1].split("]")[0]
    list_artists.append(tracks_names["name"][i] + " || " + artists)
...
X_test_names = v.transform(list(tracks_names["name"]))
